...
class Minesweeper:
    ...
    def endgame(self, victory):
        ...
        if victory:
            ...
            for row, col in self.mine_positions:
                for _ in range(3):  # Animation d'explosion # Ajouter
                    self.buttons[row][col].configure(fg_color='red') # Ajouter
                    self.root.update() # Ajouter
                    time.sleep(0.1) # Ajouter
                    self.buttons[row][col].configure(fg_color='black') # Ajouter
                    self.root.update() # Ajouter
                    time.sleep(0.1) # Ajouter
                self.buttons[row][col].configure(text='💣', fg_color='red') # Ajoute
